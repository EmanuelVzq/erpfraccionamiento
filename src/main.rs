...
#[derive(Serialize)]
struct InicioResidenteResp {
    nombre: String,
    reservas: Vec<ReservaConAreaRow>,
    pagos: Vec<PagoRow>,
}

#[derive(Serialize)]
struct PersonaPerfilResp {
   nombre: String,
   correo: Option<String>,
   numero_casa: Option<i32>, 
}

#[derive(Serialize, FromRow, Debug)]
struct MesaDirectivaMiembro {
    id_persona: i32,
    nombre: String,
    primer_apellido: String,
    segundo_apellido: Option<String>,
    correo: Option<String>,
    telefono: Option<String>,
    numero_residencia: Option<i32>,
    cargo: String, // nombre del rol (mesa_directiva)
}

// ====== DASHBOARD (ADMIN / MESA) ======
#[derive(Serialize, FromRow, Debug)]
struct AvisosMesItem {
    anio: i32,
    mes: i32,
    total: i64,
}
...
#[derive(Serialize, Debug)]
struct DashboardAdminResp {
    reservas: Vec<ReservaConAreaRow>,
    pagos: Vec<PagoRow>,
    avisos_por_mes: Vec<AvisosMesItem>,
}

#[derive(Serialize, Debug)]
struct DashboardMesaResp {
    reservas: Vec<ReservaConAreaRow>,
    pagos: Vec<PagoRow>,
    avisos_por_mes: Vec<AvisosMesItem>,
}

// 🔹 Historial de reservas (todas las áreas + persona)
#[derive(Serialize, sqlx::FromRow, Debug)]
pub struct ReservaHistorial {
    pub no_reserva: i32,
...
// GET /pagos/historial_all   (admin: todos los pagos NO pendientes)
async fn obtener_pagos_historial_all(pool: web::Data<PgPool>) -> impl Responder {
    let res = sqlx::query_as::<_, PagoRow>(
        r#"
        SELECT
            no_transaccion,
...
// GET /pagos/historial_todos  -> ADMIN ve todos los pagos
async fn obtener_pagos_historial_todos(
    pool: web::Data<PgPool>,
) -> impl Responder {
    let res = sqlx::query_as::<_, PagoRow>(
        r#"
...
// GET /pagos/historial_mantenimiento  -> MESA ve solo residentes + mantenimiento
async fn obtener_pagos_historial_mantenimiento(
    pool: web::Data<PgPool>,
) -> impl Responder {
    let res = sqlx::query_as::<_, PagoRow>(
        r#"
        SELECT
            pgs.no_transaccion,
            pgs.fecha_transaccion,
            pgs.id_persona,
            pgs.id_usuario_registro,
            pgs.id_tipo_cuota,
            pgs.cve_tipo_pago,
            pgs.total::text as total,
            pgs.estado
        FROM pagos pgs
        JOIN persona per ON per.id_persona = pgs.id_persona
        JOIN persona_rol pr ON pr.id_persona = per.id_persona
        JOIN rol r ON r.id_rol = pr.id_rol
        WHERE lower(r.nombre) = 'residente'
          AND pgs.id_tipo_cuota = 1         -- mantenimiento
          AND lower(pgs.estado) <> 'pendiente'
        ORDER BY pgs.fecha_transaccion DESC
        "#,
    )
    .fetch_all(pool.get_ref())
    .await;
...
async fn confirmar_pago(
    ...
    println!("/pagos/confirmar llamado con {:?}", body.payment_intent_id);

    let secret_key = match env::var("STRIPE_SECRET_KEY") {
        Ok(v) => v,
        Err(_) => {
            println!("Falta STRIPE_SECRET_KEY");
            return HttpResponse::InternalServerError().body("Falta STRIPE_SECRET_KEY");
        }
    };
    ...
    let pi_id: PaymentIntentId = match body.payment_intent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            println!("payment_intent_id inválido: {}", body.payment_intent_id);
            return HttpResponse::BadRequest().body("payment_intent_id inválido");
        }
    };

    let pi = match PaymentIntent::retrieve(&client, &pi_id, &[]).await {
        Ok(pi) => {
            println!("PaymentIntent recuperado: id={} status={:?}", pi.id, pi.status);
            pi
        }
        Err(e) => {
            println!("Stripe retrieve error: {e:?}");
            return HttpResponse::BadRequest()
                .body(format!("No se pudo recuperar PaymentIntent: {e:?}"));
        }
    };

    if pi.status != PaymentIntentStatus::Succeeded {
        println!("PaymentIntent no está SUCCEEDED: {:?}", pi.status);
        return HttpResponse::BadRequest()
    ...
    let mut tx = match pool.begin().await {
        ...
        Err(e) => {
            println!("TX error: {e}");
            return HttpResponse::InternalServerError().body(format!("TX error: {e}"));
        }
    };

    // Verificar si ya existe un pago con ese payment_intent
    let ya_res: Result<i64, sqlx::Error> = sqlx::query_scalar(
    ...
    let ya = match ya_res {
        ...
        Err(e) => {
            let _ = tx.rollback().await;
            println!("Error consultando pagos por payment_intent_id: {e}");
            return HttpResponse::InternalServerError()
                .body(format!("Error consultando pagos: {e}"));
        }
    };

    println!("pagos con ese stripe_payment_intent_id = {}", ya);

    if ya > 0 {
        let _ = tx.commit().await;
        println!("Pago ya registrado, devolviendo OK");
        return HttpResponse::Ok().body("Ya registrado");
    }

    // Validar que la persona exista, pero sin ocultar errores
    let existe_persona_res: Result<i64, sqlx::Error> = sqlx::query_scalar(
        "SELECT COUNT(*)::BIGINT FROM persona WHERE id_persona=$1",
    )
    .bind(id_persona)
    .fetch_one(&mut *tx)
    .await;

    let existe_persona = match existe_persona_res {
        Ok(v) => v,
        Err(e) => {
            let _ = tx.rollback().await;
            println!("Error consultando persona({}): {e}", id_persona);
            return HttpResponse::InternalServerError()
                .body(format!("Error verificando persona: {e}"));
        }
    };

    println!("existe_persona({}) = {}", id_persona, existe_persona);

    if existe_persona == 0 {
        let _ = tx.rollback().await;
        println!("id_persona no existe en BD");
        return HttpResponse::BadRequest()
            .body(format!("id_persona no existe: {}", id_persona));
    }

    println!("Insert/Update en tabla pagos...");
    ...
    {
        let _ = tx.rollback().await;
        println!("Insert/Update pagos error: {e}");
        return db_err_to_http(e, "Insert pagos error");
    }

    println!("Borrando detalle previo e insertando nuevo...");
    ...
    {
        let _ = tx.rollback().await;
        println!("Insert pago_detalle error: {e}");
        return db_err_to_http(e, "Insert pago_detalle error");
    }
    ...
    println!("Pago registrado correctamente en BD");
    HttpResponse::Ok().json(serde_json::json!({
        "ok": true,
...
// GET /inicio/residente/{id_persona}
async fn inicio_residente(
    pool: web::Data<PgPool>,
    path: web::Path<i32>,
) -> impl Responder {
    let id_persona = path.into_inner();

    // 1) Obtener nombre de la persona
    let persona_row = sqlx::query_as::<_, PersonaNombreRow>(
        r#"
    ...
    let persona_row = match persona_row {
        Ok(Some(p)) => p,
        Ok(None) => {
            return HttpResponse::NotFound()
                .body(format!("No existe persona con id_persona={}", id_persona));
        }
        Err(e) => {
            return HttpResponse::InternalServerError()
                .body(format!("Error obteniendo persona: {e}"));
        }
    };

    let nombre_completo = match persona_row.segundo_apellido {
        Some(seg) if !seg.is_empty() => {
            format!("{} {} {}", persona_row.nombre, persona_row.primer_apellido, seg)
        }
        _ => format!("{} {}", persona_row.nombre, persona_row.primer_apellido),
    };

    // 2) Reservas recientes (últimas 10)
    let reservas_res = sqlx::query_as::<_, ReservaConAreaRow>(
        r#"
        SELECT
          r.no_reserva,
          r.cve_area,
    ...
    let reservas = match reservas_res {
        Ok(list) => list,
        Err(e) => {
            return HttpResponse::InternalServerError()
                .body(format!("Error obteniendo reservas: {e}"));
        }
    };

    // 3) Pagos recientes (últimos 10, cualquier estado)
    let pagos_res = sqlx::query_as::<_, PagoRow>(
        r#"
        SELECT
            no_transaccion,
            fecha_transaccion,
    ...
    let pagos = match pagos_res {
        Ok(list) => list,
        Err(e) => {
            return HttpResponse::InternalServerError()
                .body(format!("Error obteniendo pagos: {e}"));
        }
    };

    // 4) Respuesta unificada para el dashboard del residente
    HttpResponse::Ok().json(InicioResidenteResp {
...
async fn obtener_persona_por_id(
    pool: web::Data<PgPool>,
    path: web::Path<i32>,
) -> impl Responder {
    let id_persona = path.into_inner();

    let res = sqlx::query_as::<_, Persona>(
        r#"
        SELECT id_persona, nombre, primer_apellido, segundo_apellido,
               correo, telefono, no_residencia
        FROM persona
    ...
    match res {
        Ok(Some(p)) => {
            ...
            let mut nombre = p.nombre.clone();
            if !p.primer_apellido.is_empty() {
                nombre = format!("{} {}", nombre, p.primer_apellido);
            }
            if let Some(seg) = p.segundo_apellido.clone() {
                if !seg.is_empty() {
                    nombre = format!("{} {}", nombre, seg);
                }
            }

            let resp = PersonaPerfilResp {
                nombre,
                correo: p.correo.clone(),
                numero_casa: p.no_residencia,
            };
...
// GET /mesa_directiva/miembros
async fn obtener_mesa_directiva(pool: web::Data<PgPool>) -> impl Responder {
    let res = sqlx::query_as::<_, MesaDirectivaMiembro>(
        r#"
        SELECT
            p.id_persona,
            p.nombre,
            ...
            p.correo,
            p.telefono,
            p.no_residencia AS numero_residencia,
            r.nombre AS cargo
        FROM persona p
        JOIN persona_rol pr
        ...
        WHERE lower(r.nombre) = 'mesa_directiva'
    ...
    match res {
        ...
        Err(e) => db_err_to_http(e, "Error obtener_mesa_directiva"),
    }
}

// GET /dashboard/admin
async fn dashboard_admin(pool: web::Data<PgPool>) -> impl Responder {
    // 1) Reservas recientes de TODOS
    let reservas_res = sqlx::query_as::<_, ReservaConAreaRow>(
        r#"
        SELECT
          r.no_reserva,
    ...
    let reservas = match reservas_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo reservas dashboard_admin"),
    };

    // 2) Pagos recientes de TODOS
    let pagos_res = sqlx::query_as::<_, PagoRow>(
        r#"
        SELECT
            no_transaccion,
    ...
    let pagos = match pagos_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo pagos dashboard_admin"),
    };

    // 3) Cantidad de avisos por mes (últimos 12)
    let avisos_res = sqlx::query_as::<_, AvisosMesItem>(
        r#"
        SELECT
            EXTRACT(YEAR  FROM a.creado_en)::INT AS anio,
            EXTRACT(MONTH FROM a.creado_en)::INT AS mes,
            COUNT(*)                             AS total
        FROM aviso a
        GROUP BY anio, mes
    ...
    let avisos_por_mes = match avisos_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo avisos_por_mes admin"),
    };

    HttpResponse::Ok().json(DashboardAdminResp {
        reservas,
        pagos,
        avisos_por_mes,
    })
...
// GET /dashboard/mesa/{id_persona}
async fn dashboard_mesa(
    pool: web::Data<PgPool>,
    path: web::Path<i32>,
) -> impl Responder {
    let id_persona = path.into_inner();

    // 1) Reservas recientes (pueden ser globales, igual que admin)
    let reservas_res = sqlx::query_as::<_, ReservaConAreaRow>(
        r#"
        SELECT
          r.no_reserva,
    ...
    let reservas = match reservas_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo reservas dashboard_mesa"),
    };

    // 2) Pagos recientes (también globales)
    let pagos_res = sqlx::query_as::<_, PagoRow>(
        r#"
        SELECT
            no_transaccion,
    ...
    let pagos = match pagos_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo pagos dashboard_mesa"),
    };

    // 3) Cantidad de avisos por mes SOLO de ese miembro de mesa_directiva
    //    Se filtra por persona a través de la tabla usuario.
    let avisos_res = sqlx::query_as::<_, AvisosMesItem>(
        r#"
        SELECT
            EXTRACT(YEAR  FROM a.creado_en)::INT AS anio,
    ...
    let avisos_por_mes = match avisos_res {
        Ok(list) => list,
        Err(e) => return db_err_to_http(e, "Error obteniendo avisos_por_mes mesa"),
    };

    HttpResponse::Ok().json(DashboardMesaResp {
        reservas,
        pagos,
        avisos_por_mes,
    })
...
// GET /reservas/historial  (nuevo: historial global con persona)
async fn get_reservas_historial(
    pool: web::Data<PgPool>,
) -> impl Responder {
    let query = r#"
        SELECT
            r.no_reserva AS no_reserva,
            a.nombre AS area_nombre,
            r.fecha_reserva,
            r.hora_inicio,
            r.hora_fin,
            r.estado,
            (p.nombre || ' ' || p.primer_apellido || ' ' || COALESCE(p.segundo_apellido, '')) AS nombre_persona
        FROM reservas r
        JOIN area_comun a
            ON a.cve_area = r.cve_area
        JOIN persona p
            ON p.id_persona = r.id_persona_solicitante
        ORDER BY r.fecha_reserva DESC, r.hora_inicio DESC
    "#;

    let res = sqlx::query_as::<_, ReservaHistorial>(query)
        .fetch_all(pool.get_ref())
        .await;
    ...
    match res {
        ...
        Err(e) => {
            eprintln!("[/reservas/historial] DB error: {e:?}");
            HttpResponse::InternalServerError().body("Error al obtener historial de reservas")
        }
    }
}
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/persona/{id_persona}", web::get().to(obtener_persona_por_id))
            // LOGIN
            ...
            //   MESA DIRECTIVA
            .route("/mesa_directiva/miembros", web::get().to(obtener_mesa_directiva))
            // DISPOSITIVOS
            ...
            .route("/pagos/historial_todos", web::get().to(obtener_pagos_historial_todos))
            .route("/pagos/historial_mantenimiento", web::get().to(obtener_pagos_historial_mantenimiento))
            ...
            .route("/reservas/historial", web::get().to(get_reservas_historial))
            // DASHBOARD INICIO RESIDENTE
            .route("/inicio/residente/{id_persona}", web::get().to(inicio_residente))
            // DASHBOARDS
            .route("/dashboard/admin", web::get().to(dashboard_admin))
            .route("/dashboard/mesa/{id_persona}", web::get().to(dashboard_mesa))
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
